# Änderungshistorie - MII IG Kerndatensatz-Modul Onkologie v2027.0.0-ballot.rc3
...
* **`BREAKING`** **Canonical-URLs an die Namenskonvention angeglichen (10 Artefakte)**: sieben Therapie-ValueSets erhielten das fehlende `onko-`-Segment, drei logische Modelle verloren den Simplifier-Altpfad `…/LogicalModel/<Name>` — vollständige Alt→Neu-Tabelle und Migrationshinweise auf der Seite [Migration von v2026](migration-v2026.md), Eintrag 10. Instanzen sind nicht betroffen (sie tragen diese Canonicals nie); Profil-Bindings folgen automatisch.
...
* **Neue Seiten**: [Therapieziele](therapieziele.md) (Verhältnis zur BIH-CEI-Therapieziele-Spezifikation) und die vervollständigte [Anleitung für Forschende](researcher-guidance.md) (Meldeverzug, Standort-Fragmentierung); alle Template-TODO-Boxen entfernt (Security & Privacy, Übersetzungsstand, Versionierung, Downloads).
...
Diese Änderungen erfordern möglicherweise Anpassungen in bestehenden Implementierungen. Jeder Eintrag ist auf der Seite [Migration von v2026](migration-v2026.md) einzeln erläutert — mit Auswirkung auf Bestandsdaten, Auswirkung auf Implementierungen und konkretem Migrationsweg:
...
* **`BREAKING`** **Abhängigkeiten auf die 2027-Ballot-Linie**: `kerndatensatz.meta` 2027.0.0-ballot.rc3, `kerndatensatz.base` 2027.0.0-ballot.rc1, `de.basisprofil.r4` 1.6.0, `kerndatensatz.biobank` und `kerndatensatz.molgen` je 2027.0.0-ballot.rc2, `kerndatensatz.studie` 2027.0.0-ballot.rc1, `kerndatensatz.medikation` 2027.0.0-ballot.rc5. Neu hinzugekommen: `kerndatensatz.laborbefund` 2027.0.0-ballot.rc3 (für das Tumormarker-Profil) und `hl7.fhir.uv.genomics-reporting` 3.0.0.
...
* `note` **Known Issue — ~4.100 ATC-QA-Meldungen sind ein Publisher-Bug, keine ungültigen Codes**: Der IG Publisher validiert enumerierte ValueSet-Codes über einen `$batch-validate-code`-Dialekt ohne `url`-Parameter je Validierung; der MII-Terminologieserver lehnt jede Anfrage korrekt ab („must specify either valueSet or url"), was der Publisher als „code not valid in the value set 'null'" meldet. Die Codes sind serverseitig als gültig verifiziert (z. B. `L01XE01@2020` → Imatinib). Betrifft die Jahres-Substanzen-ValueSets und ATC-Übergangs-Maps; unverändert reproduziert mit Publisher 2.3.2 und 2.3.4, Upstream-Issue folgt.
...
* `feat` **Frühere Tumorerkrankungen Profil**: Neues [FruehereTumorerkrankungCondition](StructureDefinition-mii-pr-onko-fruehere-tumorerkrankung.md) Profil zur Erfassung anamnestischer Tumorerkrankungen mit Mapping zu oBDS 5.9 
...
* **Erweiterte [CarePlan](StructureDefinition-mii-pr-onko-tumorkonferenz.md)** zur Unterstützung sowohl bisheriger oBDS-Darstellung der Tumorkonferenzen als auch komplexere Darstellung von Therapieempfehlungen wie im Modul Molekulares Tumorboard
* **Grund**: FHIR R4 Invariant cpl-3 verhindert gleichzeitige Nutzung von `activity.detail.code` und `activity.reference`
* **Lösung**: Activity-Slicing mit `obds` (Standard oBDS 19.1 Kategorisierung) und `extended` ([TherapieempfehlungKombinationstherapieRequestGroup](StructureDefinition-mii-pr-onko-therapieempfehlung-kombinationstherapie.md)-basierte Protokolle) Slices
...
* **usedCode-Implementierung**: Strukturierte Dokumentation von Therapieprotokollen in [SystemischeTherapieProcedure](StructureDefinition-mii-pr-onko-systemische-therapie.md) 
...
* **Dual-Coding-Support**: [SystemischeTherapieMedicationStatement](StructureDefinition-mii-pr-onko-systemische-therapie-medikation.md) erweitert um UNII-Slice zusätzlich zum bestehenden ATC-Slice 
...
### OperationProcedure Erweiterungen
...
* **Neue Extension**: [ExtensionUrgencyOperation](StructureDefinition-mii-ex-onko-operation-urgency.md) zur Erfassung der Eingriffsmodalität 
...
* **Integration**: Extension in allgemeines [OperationProcedure](StructureDefinition-mii-pr-onko-operation.md) Profil integriert für breite Anwendbarkeit
...
* **oBDS 2014 CodeSystem Integration**: Unterstützung für oBDS 2014 Zielgebiet-Definitionen zur Abwärtskompatibilität in [StrahlentherapieProcedure](StructureDefinition-mii-pr-onko-strahlentherapie.md) 
...
* **Mamma**: Vollständige Implementierung des [MammaModule](profiles.md) 
* **Estrogen-Rezeptorstatus**: [MammaEstrogenRezeptorstatus](StructureDefinition-mii-pr-onko-mamma-rezeptorstatus-estrogen.md) mit dualer Kodierung (oBDS/S3-Leitlinien) und Komponenten für Anteil positiver Zellen und Färbeintensität
* **Progesteron-Rezeptorstatus**: [MammaProgesteronRezeptorstatus](StructureDefinition-mii-pr-onko-mamma-rezeptorstatus-progesteron.md) mit identischer Struktur zum Estrogen-Status
* **Menopause-Status**: [MammaMenopauseStatus](StructureDefinition-mii-pr-onko-mamma-menopause-status.md) mit oBDS 2021-konformer Subsumierung (perimenopausal → prämenopausal)
* **Präoperative Markierung**: [MammaPraeoperativeMarkierung](StructureDefinition-mii-pr-onko-mamma-praeoperative-markierung.md) für verschiedene Markierungsmodalitäten (Draht, Seeds, magnetisch)
* **Mamma-Operation**: [MammaOperation](StructureDefinition-mii-pr-onko-mamma-operation.md) mit SNOMED CT und OPS ValueSets
* **Hinweise**: Her2Neu im Molecular Tumorboard-Profil; Tumorgröße im Histologie-Modul; Studienteilnahme in oBDS 2021 abgedeckt
 
* **Prostata**: Vollständige Implementierung des [ProstataModule](profiles.md) 
* **PSA**: [ProstataPSA](StructureDefinition-mii-pr-onko-prostate-psa.md) mit LOINC 2857-1 Kodierung
* **Anzahl Stanzen**: [ProstataAnzahlStanzen](StructureDefinition-mii-pr-onko-prostate-anzahl-stanzen.md) (LOINC 33743-6)
* **Anzahl positive Stanzen**: [ProstataAnzahlPositiveStanzen](StructureDefinition-mii-pr-onko-prostate-anzahl-positive-stanzen.md) (LOINC 33746-9)
* **Karzinom-Befall Stanze**: [ProstataKarzinomBefallStanze](StructureDefinition-mii-pr-onko-prostate-ca-befall-stanze.md) (LOINC 33748-5)
* **Gleason Score/Grade Group**: [ProstataGleasonScoreGradeGroup](StructureDefinition-mii-pr-onko-prostate-gleason-grade-group.md) (LOINC 44648-7)
* **Gleason Patterns**: Primäre und sekundäre Gleason-Grad-Komponenten
* **Clavien-Dindo**: [ProstataClavienDindo](StructureDefinition-mii-pr-onko-prostate-clavien-dindo.md) mit dualer Kodierung (SNOMED CT + oBDS)
 
* **Kolorektales Karzinom**: Vollständige Implementierung des [KRKModule](profiles.md) 
...
* **Malignes Melanom**: Vollständige Implementierung des [MalignesMelanomModule](profiles.md) 
* **Breslow-Tiefe**: [MelanomBreslowTiefe](StructureDefinition-mii-pr-onko-melanom-breslow-tiefe.md) (SNOMED CT 106243009)
* **Ulzeration**: [MelanomUlzeration](StructureDefinition-mii-pr-onko-melanom-ulzeration.md) (oBDS MM4)
* **Sicherheitsabstand**: [MelanomSicherheitsabstand](StructureDefinition-mii-pr-onko-melanom-sicherheitsabstand.md) (oBDS MM1)
* **LDH**: [MelanomLDH](StructureDefinition-mii-pr-onko-melanom-ldh.md) Laktatdehydrogenase als prognostischer Marker
* **Melanom-Exzision**: **MelanomExzision** mit SNOMED CT Codes für Hautexzisionen
* **Bundle-Beispiel**: [MelanomBundle](profiles.md) demonstriert vollständigen Melanom-Workflow
 
* **Bundle-Beispiele**: Für alle organspezifischen Module ([MammaBundle](profiles.md), **ProstataBundle**, [KRKBundle](profiles.md), [MelanomBundle](profiles.md)) stehen vollständige Transaktions-Bundles zur Verfügung, die alle zugehörigen Profile in einem server-konsumierbaren Format demonstrieren
