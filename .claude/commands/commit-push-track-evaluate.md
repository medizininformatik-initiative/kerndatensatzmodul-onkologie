...
3. **Monitor CI validation**:
   ...
   RUN_ID=$(gh api repos/medizininformatik-initiative/kerndatensatzmodul-onkologie/actions/runs \
     --jq ".workflow_runs | map(select(.head_sha == \"$COMMIT_SHA\")) | .[0].id")

   # Poll until complete (use run_in_background for this)
   sleep 30 && gh api repos/medizininformatik-initiative/kerndatensatzmodul-onkologie/actions/runs/$RUN_ID \
...
- This command provides detailed analysis with categorization
...
Some validation errors are **expected** due to terminology server limitations:

### Wrong_Display_Name Errors
These may occur because terminology servers don't always have German translations.

**These are NOT bugs** - they're expected behavior per the MII-controlled terminology strategy.
