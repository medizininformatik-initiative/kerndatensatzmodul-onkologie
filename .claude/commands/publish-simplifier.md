...
```bash
# Navigate to project
cd /Users/thome/code/fhir-profiling/kerndatensatzmodul-onkologie

# Publish package
simplifier push \
  --project medizininformatik-initiative.kerndatensatz.onkologie \
  --version [VERSION] \
...
```bash
fhir publish \
  --project MII/kerndatensatzmodul-onkologie \
  --version [VERSION]
```

**Option C: Manual Upload Instructions**
If CLI tools unavailable, provide step-by-step manual instructions:
1. Navigate to https://simplifier.net/medizininformatik-initiative.kerndatensatz.onkologie
...
**URL**: https://simplifier.net/medizininformatik-initiative.kerndatensatz.onkologie/~packages
**Install Command**:
  fhir install de.medizininformatikinitiative.kerndatensatz.onkologie@[VERSION]
```
